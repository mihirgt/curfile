...
public class TagRuleLoader {
  ...
  public static List<CURTagRule> loadRulesFromYaml(String configPath) {
    ...
    try {
      ...
      InputStream inputStream =
          TagRuleLoader.class.getClassLoader().getResourceAsStream(configPath);

      if (inputStream == null) {
        logger.warn(
            "Could not find tagging rules config file: {}. Using empty rules list.", configPath);
        return rules;
      }

      Map<String, Object> config = yaml.load(inputStream);
      List<Map<String, Object>> taggingRules =
          (List<Map<String, Object>>) config.get("tagging_rules");
  ...
  private static CURTagRule.Condition parseCondition(Map<String, Object> conditionMap) {
    ...
    if (conditionMap.containsKey("logical_operator") && conditionMap.containsKey("conditions")) {
      ...
      List<Map<String, Object>> subConditions =
          (List<Map<String, Object>>) conditionMap.get("conditions");
    ...
    else if (conditionMap.containsKey("field")
        && conditionMap.containsKey("operator")
        && conditionMap.containsKey("value")) {
