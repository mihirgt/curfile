...
/**
 * Configuration class for Google Cloud Platform settings.
 * Handles authentication and connection settings for GCS and BigQuery.
...
public class GCPConfig {
    ...
    public SparkConf configureSparkForGCP(SparkConf conf) {
        ...
        if (Files.exists(Paths.get(k8sServiceAccountPath))) {
            ...
            conf.set("spark.hadoop.google.cloud.auth.service.account.json.keyfile", k8sServiceAccountPath);
            ...
            conf.set("spark.hadoop.google.cloud.auth.service.account.enable", "true");
            conf.set("spark.hadoop.google.cloud.auth.service.account.json.keyfile", serviceAccountKeyPath);
        } else {
