...
/**
 * CURIngestionApp - A Spark application that reads CUR (Cost and Usage Report) files from Google
 * Cloud Storage and ingests them into BigQuery.
 *
 * <p>This application is designed to be run in a Kubernetes environment using Helm for
 * configuration. Configuration is read from a YAML file mounted at /etc/cur-ingestion/config.yaml.
 */
public class CURIngestionApp {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      SparkConf sparkConf = new SparkConf().setAppName("CUR Ingestion to BigQuery");
      ...
      } else if (!serviceAccountKeyPath.isEmpty()
          && Files.exists(Paths.get(serviceAccountKeyPath))) {
      ...
      Tuple2<Dataset<Row>, Dataset<Row>> taggingResult =
          CURDataTransformer.applyDirectTags(transformedData, rulesFile);
      ...
      transformedData =
          transformedData.withColumn("ingestion_timestamp", functions.current_timestamp());

      resourceTagHistory =
          resourceTagHistory.withColumn("ingestion_timestamp", functions.current_timestamp());
      ...
      System.out.println(
          "CUR data successfully ingested into BigQuery table: " + projectId + ":" + bigQueryTable);
  ...
  private static Dataset<Row> readCURFile(SparkSession spark, String gcsPath) {
    ...
    if (gcsPath.endsWith(".csv") || gcsPath.endsWith(".csv.gz")) {
      ...
      curData = spark.read().option("multiline", "true").option("mode", "PERMISSIVE").json(gcsPath);
      ...
      try {
        ...
        System.out.println(
            "Failed to use predefined schema, falling back to CSV with header inference: "
                + e.getMessage());
        curData =
            spark
                .read()
  ...
  /**
   * Write the processed data to BigQuery using an upsert operation This handles cases where the
   * same row might exist with different tags or cost values
   *
   ...
   */
  private static void writeToBigQuery(
      Dataset<Row> data, String projectId, String tableId, String tempBucket) {
    ...
    try {
      ...
      data.write().format("bigquery").options(tempOptions).mode(SaveMode.Overwrite).save(projectId);
      ...
      data.write().format("bigquery").options(options).mode(SaveMode.Append).save(projectId);
    }
  }
}
