...
public class CURDataTransformerTest {
  ...
  public void setUp() {
    ...
    spark =
        SparkSession.builder().appName("CURDataTransformerTest").master("local[2]").getOrCreate();

    // Load sample CUR data
    sampleCurData =
        spark.read().option("header", "true").option("inferSchema", "true").csv(SAMPLE_CUR_PATH);
  }
  ...
  public void testApplyDataQualityChecks() {
    ...
    if (CURDataTransformer.containsColumn(sampleCurData, "line_item_usage_account_id")) {
      ...
      long nullAccountIdCount =
          sampleCurData.filter(sampleCurData.col("line_item_usage_account_id").isNull()).count();
    ...
    if (CURDataTransformer.containsColumn(checkedData, "line_item_unblended_cost")) {
      long nullCostCount =
          checkedData.filter(checkedData.col("line_item_unblended_cost").isNull()).count();
  ...
  public void testContainsColumn() {
    ...
    assertThat(CURDataTransformer.containsColumn(sampleCurData, "line_item_usage_account_id"))
        .isTrue();
