...
public class CURFileFormatTest {
  ...
  public void setUp() {
    ...
    spark = SparkSession.builder().appName("CURFileFormatTest").master("local[2]").getOrCreate();
  }
  ...
  public void testReadCURFileCSV() throws Exception {
    ...
    Method readCURFileMethod =
        CURIngestionApp.class.getDeclaredMethod("readCURFile", SparkSession.class, String.class);
    ...
    assertThat(curData.columns())
        .contains("identity_line_item_id", "line_item_usage_account_id", "line_item_product_code");
  }
  ...
  public void testReadCURFileJSON() throws Exception {
    ...
    assertThat(ec2Row.getString(curData.schema().fieldIndex("line_item_product_code")))
        .isEqualTo("AmazonEC2");
    assertThat(ec2Row.getString(curData.schema().fieldIndex("line_item_usage_type")))
        .isEqualTo("BoxUsage:m5.large");

    // Check that we can handle null values properly
    Row taxRow = curData.filter("identity_line_item_id = '2004'").first();
    assertThat(taxRow.getString(curData.schema().fieldIndex("line_item_line_item_type")))
        .isEqualTo("Tax");
  ...
  public void testTransformationWithDifferentFormats() throws Exception {
    ...
    if (CURDataTransformer.containsColumn(checkedCsvData, "line_item_unblended_cost")) {
      long nullCostCount =
          checkedCsvData.filter(checkedCsvData.col("line_item_unblended_cost").isNull()).count();
    ...
    if (CURDataTransformer.containsColumn(checkedJsonData, "line_item_unblended_cost")) {
      long nullCostCount =
          checkedJsonData.filter(checkedJsonData.col("line_item_unblended_cost").isNull()).count();
